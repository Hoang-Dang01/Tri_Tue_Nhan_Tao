...
Data splitting
--------------
Both the train/test holdout and the CV folds are grouped by registered domain
(GroupShuffleSplit / StratifiedGroupKFold): no domain ever appears on both
sides of a split. A random split leaks heavily here (~83% of test URLs share a
domain with training URLs), inflating F1 from ~0.93 to ~0.98 — the model would
just memorise domain strings via TF-IDF instead of learning URL structure.
...
def _registered_domain(url: str) -> str:
    """
    Approximate registered domain (last two labels of the hostname), used as
    the group key so related URLs never straddle a train/test boundary.
    """
...
def _param_grid(fast: bool = False) -> dict:
    ...
    fast=True           : 12 combinations  — quick run, ~5-15 min.
    """
    if fast:
        return {
...
def main() -> None:
    ...
    # ── Train / test split (grouped by registered domain — no leakage) ───────
    groups = X["url"].map(_registered_domain)
    logger.info("Unique registered domains: %d", groups.nunique())
    ...
    logger.info(
        "Train: %d rows | Test: %d rows | domain overlap train<->test: %.1f%%",
        len(X_train), len(X_test), overlap * 100,
    )
    ...
    payload = {
        ...
        "meta": {
            ...
            "split": "group_by_registered_domain",
            "malicious_label": malicious_label,
            "best_threshold": best_threshold,
